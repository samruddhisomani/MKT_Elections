...
masterdata = read.csv('GitHub/MKT_Elections/data/MasterData2.csv', header=TRUE)
...
scores = predict(model, comps=1:75)
inv_scores = inv.logit(scores)
view = cbind(inv_scores, masterdata$rep_trans)
colnames(view) <- c("predict", "actual")
sqrt(mean((view[,1]-view[,2])^2))

cbind1=cbind(view, bindstate[,2])
newbind=cbind(bindstate,view)
...
ggplot(bindstate, aes(map_id = as.factor(State))) + 
  ...
  geom_map(map = fort,aes(fill=newbind[,6]),color='black') + 
